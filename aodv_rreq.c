			   ...
			   u_int32_t dest_seqno, struct in_addr orig_addr)
{
    ...
    char *device;
    struct ifreq ifr;
    device = DEV_NR(0).ifname;
    strcpy(ifr.ifr_name,device);
    Bwinfo *Bwi_c1 = NULL,*Bwi_c2 = NULL,*p_c1,*p_c2;
    getData(&Bwi_c1,&ifr);
    usleep(1000000);
    getData(&Bwi_c2,&ifr);
    p_c1 = Bwi_c1;
    p_c2 = Bwi_c2;
    //printf("----huasi----p_c1->recvBytes = %.2f\n",(float)(p_c1->recvBytes)/1024);
    //printf("----huasi----p_c2->recvBytes = %.2f\n",(float)(p_c2->recvBytes)/1024);
    rreq->Bandwidth = (float)(p_c2->recvBytes - p_c1->recvBytes)/1024;
    //printf("----huasisi---rreq->Bandwidth = %.2f\n",rreq->Bandwidth);
    DEBUG(LOG_DEBUG,0,"rreq->Bandwidth = %.2f",rreq->Bandwidth);
    rreq->Bmin = 10.00;
    /*if(rreq->Bmin >= rreq->Bandwidth)
        printf("Bmin is bigger than Bandwidth\n");
    else
...
//	printf("-----------huasisi----Bandwidth1 = %.2f\n",(float)rreq->Bandwidth);
			   ...
			   unsigned int ifindex)
{
    ...
    device = DEV_NR(0).ifname;
    strcpy(ifr.ifr_name,device);
    Bwinfo *Bwi_p1 = NULL,*Bwi_p2 = NULL,*p_p1,*p_p2;
    getData(&Bwi_p1,&ifr);
    usleep(1000000);
    getData(&Bwi_p2,&ifr);
    p_p1 = Bwi_p1;
    p_p2 = Bwi_p2;
    //printf("----huasi----p_p1->recvBytes = %.2f\n",(float)(p_p1->recvBytes)/1024);
    //printf("----huasi----p_p2->recvBytes = %.2f\n",(float)(p_p2->recvBytes)/1024);
    rreq_Bandwidth = (float)(p_p2->recvBytes - p_p1->recvBytes)/1024;
  //printf("----huasisi---rreq->Bandwidth = %.2f\n",rreq->Bandwidth);
    DEBUG(LOG_DEBUG,0,"rreq_Bandwidth = %.2f",rreq_Bandwidth);
    ...
    if(rreq_Bandwidth < rreq->Bmin){
        DEBUG(LOG_DEBUG,0,"the host's Bandwidth %.2f is smaller than the Bmin %.2f",rreq_Bandwidth,rreq->Bmin);
        return;
    }

    if(rreq_Bandwidth < rreq->Bandwidth){
        rreq->Bandwidth = rreq_Bandwidth;
        DEBUG(LOG_DEBUG,0,"rreq->Bandwidth : %.2f",rreq->Bandwidth);
    }
    /*---------huasisi-------*/
...
int getData(Bwinfo **Bwi,struct ifreq *ifr)
{
    Bwinfo* po;
    FILE *fpo = NULL;
    char Bwbuf[1024],*bufpo,c,*Bwtok,*Bwtemp;
    int i=0;

    *Bwi = (Bwinfo*)malloc(sizeof(Bwinfo));
    po = *Bwi;

    fpo = fopen("/proc/net/dev","r");
    if(fpo == NULL)
    {
        DEBUG(LOG_DEBUG, 0, "Could not open /proc/net/dev");
        return -1;
    }

    while(!feof(fpo)){
        for(bufpo = Bwbuf;(fread(&c,1,1,fpo)) > 0 && c !='\n' && bufpo < Bwbuf+1024;*bufpo++ =c);
        *bufpo++ = '\0';

        if(Bwbuf[0] == '\0') break;
        if(!strchr(Bwbuf,':')) continue;

        Bwtok = strtok(Bwbuf, " ");
        if(Bwtok == NULL)
        {
        DEBUG(LOG_DEBUG, 0, "no tokens in buffer.");
        return -1;
        }
        ...
        if(Bwtok != NULL)
        {
            ...
            if(Bwtemp == NULL)
            {
            DEBUG(LOG_DEBUG, 0, "error fetching interface name,token:%s\n",Bwtok);
            return -1;
            }
            if(Bwtemp - Bwtok >32)
            {
                DEBUG(LOG_DEBUG, 0, "Error fetcing interface name,buffer to small.token: %s\n",Bwtok);
                return -1;
            }
            strncpy(po->dev,Bwtok,Bwtemp-Bwtok);
            po->dev[Bwtemp -Bwtok] = '\0';
            if(strcoll(po->dev,ifr->ifr_name))
                continue;
            if(!strcoll(po->dev,ifr->ifr_name)){
                if(Bwtemp != Bwtok+strlen(Bwtok)-1)
                {
                    Bwtemp++;
                    po->recvBytes = strtol(Bwtemp,NULL,10);
                }
                Bwtok = strtok(NULL, " ");
            }

            i++;
            *Bwi = (Bwinfo*)realloc(*Bwi,sizeof(Bwinfo)*(i+1));
            po = *(Bwi)+1;
            break;
        }
   }
    memset(po->dev,0,32);
    po->recvBytes = po->recvPackets = po->transBytes = po->transPackets =-1;
    fclose(fpo);
    return 0;
}
/*---------huasisi-------*/
